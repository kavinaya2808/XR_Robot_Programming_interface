    ...
    public class Radar : MonoBehaviour
    {
        ...
        private void OnLocatableAdded(LocatableComponent locatable)
        {
            ...
            if(locatable != null && !locatableIconDictionary.ContainsKey(locatable))
            {
                ...
                // NEW: wire the Locatable to the icon updater if present
                var updater = icon.GetComponent<LocatableIconUpdater>();
                if (updater != null)
                {
                    updater.Locatable = locatable;
                    updater.Player = this.Player != null ? this.Player.transform : null;
                    // optionally set the speedSource if you have a RobotTelemetry on the locatable:
                    var rt = locatable.GetComponent<RobotTelemetry>();
                    if (rt != null) updater.speedSource = rt;
                }
